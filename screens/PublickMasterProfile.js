...
const PublickMasterProfile = ({navigation}) => {
  ...
  const [allPhoto, setAllPhoto] = useState([]),
    ...
    [timeSelectError, setTimeSelectError] = useState(false),
  ...
  if (MASTER.error) {
    ...
    return (
      <View style={{flex: 1}}>
        ...
        {(timeWasSelected || timeSelectError) && (
          <ModalWindow>
            <Text>{timeSelectError ? 'Произошла ошибка' : 'Вы записаны'}</Text>
